...
export class ProductionQueueManager {
  ...
  private static readonly PERSISTENCE_KEY = 'wonderwave_production_queue_state';
  ...
  private static loadPersistedState(): void {
    try {
      ...
      if (savedState) {
        ...
        if (this.isRunning) {
          console.log('🔄 Restoring production queue state from previous session');
          // Auto-restart if it was running
          this.getInstance().startProductionQueue();
        }
      }
    } catch (error) {
      console.error('Failed to load production queue persisted state:', error);
    }
  }
  ...
  private static persistState(): void {
    try {
      ...
      console.error('Failed to persist production queue state:', error);
    }
  }
  ...
  async startProductionQueue(): Promise<void> {
    ...
    ProductionQueueManager.persistState();
