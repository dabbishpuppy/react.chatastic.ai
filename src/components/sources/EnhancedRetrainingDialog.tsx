...
import { CheckCircle, AlertCircle, Loader2, FileText, Globe, HelpCircle, File, RefreshCw } from "lucide-react";
import { EnhancedRetrainingStatus } from "@/services/rag/retraining/enhancedRetrainingChecker";

interface EnhancedRetrainingDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  isRetraining: boolean;
  progress: any;
  retrainingNeeded: EnhancedRetrainingStatus | null;
  onStartRetraining: () => void;
  onRetryFailedSource: (sourceId: string, sourceType: string) => void;
...
export const EnhancedRetrainingDialog: React.FC<EnhancedRetrainingDialogProps> = ({
  ...
  const getCurrentStatus = () => {
    ...
    // Use training progress status directly if available and training is active
    if (trainingProgress?.status && (isRetraining || trainingProgress.status === 'completed')) {
  ...
  console.log('🔍 EnhancedRetrainingDialog render state:', {
    currentStatus,
    currentProgress,
    isRetraining,
    trainingProgressStatus: trainingProgress?.status,
  ...
  const getStatusMessage = () => {
    switch (currentStatus) {
      ...
      case 'has_failures':
        ...
        if (failedCount > 0 && normalCount > 0) {
          return `${failedCount} source${failedCount > 1 ? 's' : ''} failed and ${normalCount} need${normalCount === 1 ? 's' : ''} training. Failed sources can be retried.`;
        } else if (failedCount > 0) {
          return `${failedCount} source${failedCount > 1 ? 's' : ''} failed to process. You can retry them below.`;
        }
        return retrainingNeeded?.message || "Some sources have errors and need attention.";
  ...
  const getDialogTitle = () => {
    switch (currentStatus) {
      case 'initializing':
        const hasStartedChunking = trainingProgress?.processedChunks > 0 || trainingProgress?.totalChunks > 0;
        return hasStartedChunking ? "Agent Training in Progress" : "Initializing Training";
      case 'training':
  ...
  const handleStartTraining = async () => {
    ...
    try {
      ...
      console.log('📊 Keeping dialog open to show enhanced training progress');
    } catch (error) {
  ...
  const renderFooter = () => {
    switch (currentStatus) {
      ...
      case 'initializing':
        if (shouldShowInitializingSpinner()) {
          return (
      ...
      <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            ) : (currentStatus === 'pending' || currentStatus === 'has_failures') ? (
        ...
        <div className="flex-1 overflow-y-auto space-y-4">
            ...
            <div className="space-y-4">
              <div className="border-t pt-4">
                <h4 className="font-medium text-sm mb-3">
                  {currentStatus === 'has_failures' ? 'Sources requiring attention:' : 'Sources requiring processing:'}
                </h4>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {source.canRetry && (
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => handleRetrySource(source.id, source.type)}
          ...
          {currentStatus === 'up_to_date' && (
            <div className="text-center py-8">
              <CheckCircle className="h-12 w-12 text-green-600 mx-auto mb-4" />
              <h3 className="font-medium text-lg mb-2">Everything is up to date!</h3>
              <div className="text-sm text-gray-600">
