...
export function validateSourcePageRecord(record: any): string[] {
  ...
  if (!record.parent_source_id || typeof record.parent_source_id !== 'string') {
    errors.push(`parent_source_id must be a non-empty string, got ${typeof record.parent_source_id}`);
  }
  if (!record.customer_id || typeof record.customer_id !== 'string') {
    errors.push(`customer_id must be a non-empty string, got ${typeof record.customer_id}`);
  }
  if (!record.url || typeof record.url !== 'string') {
    errors.push(`url must be a non-empty string, got ${typeof record.url}`);
  }
  
  // Text fields that must be strings (NOT booleans)
  ...
  if (record.priority !== undefined && typeof record.priority !== 'string') {
    errors.push(`priority must be a string, got ${typeof record.priority} with value ${record.priority}`);
  }
  if (record.error_message !== undefined && typeof record.error_message !== 'string') {
    errors.push(`error_message must be a string, got ${typeof record.error_message}`);
  }
  if (record.content_hash !== undefined && typeof record.content_hash !== 'string') {
    errors.push(`content_hash must be a string, got ${typeof record.content_hash}`);
  }
  
  // Integer fields with strict validation
  if (record.retry_count !== undefined && (!Number.isInteger(record.retry_count) || record.retry_count < 0)) {
    errors.push(`retry_count must be a non-negative integer, got ${record.retry_count} (type: ${typeof record.retry_count})`);
  }
  if (record.max_retries !== undefined && (!Number.isInteger(record.max_retries) || record.max_retries < 0)) {
    errors.push(`max_retries must be a non-negative integer, got ${record.max_retries} (type: ${typeof record.max_retries})`);
  }
  if (record.content_size !== undefined && (!Number.isInteger(record.content_size) || record.content_size < 0)) {
    errors.push(`content_size must be a non-negative integer, got ${record.content_size} (type: ${typeof record.content_size})`);
  }
  if (record.chunks_created !== undefined && (!Number.isInteger(record.chunks_created) || record.chunks_created < 0)) {
    errors.push(`chunks_created must be a non-negative integer, got ${record.chunks_created} (type: ${typeof record.chunks_created})`);
  }
  if (record.duplicates_found !== undefined && (!Number.isInteger(record.duplicates_found) || record.duplicates_found < 0)) {
    errors.push(`duplicates_found must be a non-negative integer, got ${record.duplicates_found} (type: ${typeof record.duplicates_found})`);
  }
  if (record.processing_time_ms !== undefined && (!Number.isInteger(record.processing_time_ms) || record.processing_time_ms < 0)) {
    errors.push(`processing_time_ms must be a non-negative integer, got ${record.processing_time_ms} (type: ${typeof record.processing_time_ms})`);
  }
  
  // Real/float fields
  if (record.compression_ratio !== undefined && (typeof record.compression_ratio !== 'number' || record.compression_ratio < 0)) {
    errors.push(`compression_ratio must be a non-negative number, got ${typeof record.compression_ratio} with value ${record.compression_ratio}`);
  }
  
  // Timestamp fields - allow both Date objects and ISO strings
  ...
  // Check for boolean values where strings are expected (common source of the error)
  const stringFields = ['status', 'priority', 'error_message', 'content_hash'];
  stringFields.forEach(field => {
    if (record[field] !== undefined && typeof record[field] === 'boolean') {
      errors.push(`CRITICAL: ${field} is a boolean (${record[field]}) but should be a string`);
    }
  });
